...
function Print_Offence_by_Emp(props) {
  ...
  function createHead(i,page){
    ...
    pdfHead =  [
      ...
      },
      {
        columns:[
          { text: '', style: 'header' } , 
          { text: 'บันทึกประวัติการกระทำความผิด', style: 'subheader' },
          { text: '', style: 'header' }
        ]
      },
      {
  ...
  function Toptable(j,page){
    ...
    var column = [];
      ...
      column.push('วันที่');
      column.push('ประเภท');
      column.push('รายละเอียดการกระทำผิด');
      column.push('ผลการลงโทษ');
    ...
    for(var i = 0; i < allPageData; i++) {
      ...
      if(sourceData[i] === undefined){
       ...
       /* if(page > 0){
        ...
        dataRow.push(i+(num+1));
        dataRow.push(Moment(sourceData[i].offence_date).format('DD-MM-yyyy'));
        dataRow.push(sourceData[i].offence_title);
        dataRow.push(sourceData[i].offence_detail);
        if(sourceData[i].offence_results === true){
          dataRow.push('√');
        }else if(sourceData[i].offence_results === false){
          dataRow.push('x');
        }else{
          dataRow.push('');
        }
        dataRow.push(sourceData[i].create_user);
  ...
  function createPDF(){
    ...
    return (
          ...
          <a onClick={getData}><i style={{color: "salmon"}} title="PRINT EMP Examination" class="fa fa-file fa-2x "></i> PRINT</a>
     
    ) 
  }

export default Print_Offence_by_Emp;